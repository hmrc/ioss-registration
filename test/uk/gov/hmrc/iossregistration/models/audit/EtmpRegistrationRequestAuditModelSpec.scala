...
class EtmpRegistrationRequestAuditModelSpec extends BaseSpec {
  ...
  private val request = AuthorisedMandatoryVrnRequest(FakeRequest("GET", "/"), testCredentials, userId, vrn, None)
  private implicit val dataRequest: AuthorisedMandatoryVrnRequest[AnyContent] = AuthorisedMandatoryVrnRequest(request, testCredentials, userId, vrn, None)
